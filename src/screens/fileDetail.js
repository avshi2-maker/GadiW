...
export async function renderFileDetail(container, session, docId, onBack) {
  ...
  detailContent.innerHTML = `
    ...
    <!-- Preview area (will be filled in Phase D) -->
    <div id="preview-area" style="margin-top: 24px;"></div>
  `;

  // ===== Wire buttons to placeholders for now (Phase C & D will replace) =====
  container.querySelector('#download-btn').addEventListener('click', async function () {
    var btn = this;
